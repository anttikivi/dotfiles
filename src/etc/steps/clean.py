...
from etc.config import CleanStepConfig, CleanStepOptions, Options, StepConfig
...
class CleanStep(BaseStep):
    ...
    def __call__(
        ...
        config = cast(CleanStepConfig, config)

        ui.trace("Created the configuration instance")
        ui.trace(f"Received the following configuration: {config}")

        # TODO: Merge with global defaults!
        defaults: CleanStepOptions = {}
        ui.trace("Created the `defaults` instance")
        if "defaults" in config:
            ui.debug('Found the key "defaults" in the system packages step')
            defaults = config["defaults"]

        ui.trace(
            (
                "After populating from `defaults`, `defaults` is now "
                f"{defaults}"
            )
        )

        directories: MutableMapping[str, CleanStepOptions] = {}
        ui.trace("Created the `directories` instance")

        if "directories" not in config:
            raise ValueError(
                f'the configuration for the step {self.directive} does not contain "directories"'
